...
Entries in `02` must be written inside the entries zone:
...
The marker contract must remain valid: exactly one start marker, exactly one end marker, correct order, local template preserved as scaffold. `[TEMPLATE ONLY]` blocks are scaffolding only and do not count as live freeze entries.
